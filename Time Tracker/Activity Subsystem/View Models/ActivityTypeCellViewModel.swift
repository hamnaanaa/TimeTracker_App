...
class ActivityTypeCellViewModel: ObservableObject {
    ...
    /// The optional start time of the last `TimeInterval` associated with this `ActivityType`
    /// (nil indicating there is no such `TimeInterval`)
    @Published var lastStartTime: Date?
    /// The optional end time of the last `TimeInterval` associated with this `ActivityType`
    /// (nil indicating there is no such `TimeInterval` or it's not finished yet)
    ...
    func updateTimePassed() {
        ...
        self.timePassed = TimeDisplayer(secondsPassed: isActive ?
                                            Date().timeIntervalSince(lastStartTime) :
                                            lastEndTime!.timeIntervalSince(lastStartTime))
