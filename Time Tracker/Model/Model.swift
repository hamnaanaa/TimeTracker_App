...
extension Model {
    ...
    func toggleActivityType(with id: ActivityType.ID) {
        ...
        if currentActivityType.isActive {
            ...
            guard let activeTimeInterval = activeTimeIntervals.first(
                where: { $0.activityType == currentActivityType.id }
            ),
...
extension Model {
    ...
    /// startTime might be nil, if there is no `TimeInterval` associated with the given `ActivityType`.
    /// In this case the endTime is also nil. Also, endTime might be nil, if the found `TimeInterval` is still active
