...
struct TimeDisplayer {
    ...
    mutating func convertToTime(_ secondsPassed: Double, showSeconds: Bool = false) {
        ...
        // FIXME: since every time minutes are displayed one minute is missing (probably due to the floor(..)
        // operation in Int(..), one minute should sometimes be added. Possible fix: do something about consistenly ignore the seconds,
        // maybe by normalizing all Dates to the last full minute (0 seconds)?
